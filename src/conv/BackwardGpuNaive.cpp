...
BackwardGpuNaive::BackwardGpuNaive(EasyCL *cl, LayerDimensions dim) :
    ...
    "\n"
    "    const int upstreamImage2dId = globalId / gInputArea;\n"
    "\n"
    "    const int intraImageOffset = globalId % gInputArea;\n"
    "    const int upstreamRow = intraImageOffset / gInputWidth;\n"
    "    const int upstreamCol = intraImageOffset % gInputWidth;\n"
    "\n"
    ...
    "    }\n"
    "\n"
    "    const int minFilterRow = max(0, upstreamRow + gMarginHeight - (gOutputHeight - 1));\n"
    "    const int maxFilterRow = min(gFilterHeight - 1, upstreamRow + gMarginHeight);\n"
    "    const int minFilterCol = max(0, upstreamCol + gMarginWidth - (gOutputWidth -1));\n"
    "    const int maxFilterCol = min(gFilterWidth - 1, upstreamCol + gMarginWidth);\n"
    "\n"
