...
Forward1::Forward1(EasyCL *cl, LayerDimensions dim) :
    ...
    "\n"
    "    int outputImage2Id = globalId / gOutputArea;\n"
    "    int exampleId = outputImage2Id / gNumFilters;\n"
    "    int filterId = outputImage2Id % gNumFilters;\n"
    "\n"
    "    // intraimage coords\n"
    "    int localid = globalId % gOutputArea;\n"
    "    int outputRow = localid / gOutputWidth;\n"
    "    int outputCol = localid % gOutputWidth;\n"
    "\n"
    "    global float const*inputCube = inputs + exampleId * gNumInputPlanes * gInputArea;\n"
    "    global float const*filterCube = filters + filterId * gNumInputPlanes * gFilterArea;\n"
    "\n"
    "    float sum = 0;\n"
    "    if (exampleId < numExamples) {\n"
    "        for (int inputPlaneIdx = 0; inputPlaneIdx < gNumInputPlanes; inputPlaneIdx++) {\n"
    "            global float const*inputPlane = inputCube + inputPlaneIdx * gInputArea;\n"
    "            global float const*filterPlane = filterCube + inputPlaneIdx * gFilterArea;\n"
    "            for (int u = -gHalfFilterHeight; u <= gHalfFilterHeight - gEven; u++) {\n"
    "                // trying to reduce register pressure...\n"
    "                #if gPadZeros == 1\n"
    "                    #define inputRowIdx (outputRow + u)\n"
    "                #else\n"
    "                    #define inputRowIdx (outputRow + u + gHalfFilterHeight)\n"
    "                #endif\n"
    "                global float const *inputRow = inputPlane + inputRowIdx * gInputWidth;\n"
    "                global float const *filterRow = filterPlane + (u+gHalfFilterHeight) * gFilterWidth + gHalfFilterWidth;\n"
    "                bool rowOk = inputRowIdx >= 0 && inputRowIdx < gInputHeight;\n"
    "                #pragma unroll\n"
    "                for (int v = -gHalfFilterWidth; v <= gHalfFilterWidth - gEven; v++) {\n"
    "                    #if gPadZeros == 1\n"
    "                        #define inputColIdx (outputCol + v)\n"
    "                    #else\n"
    "                        #define inputColIdx (outputCol + v + gHalfFilterWidth)\n"
    "                    #endif\n"
    "                    bool process = rowOk && inputColIdx >= 0 && inputColIdx < gInputWidth;\n"
    "                    if (process) {\n"
