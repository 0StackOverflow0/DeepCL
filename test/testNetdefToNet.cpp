...
TEST( testNetdefToNet, onefc ) {
    ...
    EXPECT_EQ( 1, fc->imageSize.width );
...
TEST( testNetdefToNet, 150n_10n ) {
    ...
    EXPECT_EQ( 1, fc1->imageSize.width );
...
//    EXPECT_EQ( "TANH", fc1->fn->getDefineName() );
    ...
    EXPECT_EQ( 1, fc2->imageSize.width );
...
TEST( testNetdefToNet, 3x32c5zmp2 ) {
    ...
    EXPECT_EQ( 128, conv->dim.inputSize.width );
...
TEST( testNetdefToNet, 2x64c3_drop_mp_100n_6n ) {
    EasyCL *cl = DeepCLGtestGlobals_createEasyCL();
    NeuralNet *net = new NeuralNet(cl);
    net->addLayer(InputLayerMaker::instance()->numPlanes(9)->imageSize(Dimensions(128, 1)));
    EXPECT_EQ(true, NetdefToNet::createNetFromNetdef( net, "2*64c3-drop-mp2-100n-6n" ) );
    net->print();
    EXPECT_EQ( 8, net->getNumLayers() );
    EXPECT_TRUE( dynamic_cast< ConvolutionalLayer * >( net->getLayer(1) ) != 0 );
    EXPECT_TRUE( dynamic_cast< ConvolutionalLayer * >( net->getLayer(2) ) != 0 );
    EXPECT_TRUE( dynamic_cast< DropoutLayer * >( net->getLayer(3) ) != 0 );
    EXPECT_TRUE( dynamic_cast< PoolingLayer * >( net->getLayer(4) ) != 0 );
    EXPECT_TRUE( dynamic_cast< FullyConnectedLayer * >( net->getLayer(5) ) != 0 );
    EXPECT_TRUE( dynamic_cast< FullyConnectedLayer * >( net->getLayer(6) ) != 0 );
    EXPECT_TRUE( dynamic_cast< SoftMaxLayer * >( net->getLayer(7) ) != 0 );
    ConvolutionalLayer* conv = dynamic_cast<ConvolutionalLayer*>(net->getLayer(1));
    EXPECT_EQ(128, conv->dim.inputSize.width);
    EXPECT_EQ(1, conv->dim.inputSize.height);
    EXPECT_EQ(false, conv->dim.padZeros);
    EXPECT_EQ(9, conv->dim.inputPlanes);
    EXPECT_EQ(64, conv->dim.numFilters);
    EXPECT_EQ(3, conv->dim.filterSize.width);
    EXPECT_EQ(1, conv->dim.filterSize.height);
...
TEST( testNetdefToNet, 2x32c7_3x32c5z ) {
    ...
    EXPECT_TRUE( dynamic_cast< SoftMaxLayer * >( net->getLayer(7) ) != 0 );
    ConvolutionalLayer *conv = dynamic_cast< ConvolutionalLayer * >( net->getLayer(1) );
    EXPECT_EQ( 19, conv->dim.inputSize.width );
    EXPECT_EQ( true, conv->dim.padZeros );
    EXPECT_EQ( 1, conv->dim.inputPlanes );
    EXPECT_EQ( 32, conv->dim.numFilters );
    EXPECT_EQ( 7, conv->dim.filterSize.width );
//    EXPECT_EQ( "RELU", conv->activationFunction->getDefineName() );
...
//    EXPECT_EQ( "RELU", conv->activationFunction->getDefineName() );
    ...
    EXPECT_EQ( 32, conv->dim.inputPlanes );
    EXPECT_EQ( 32, conv->dim.numFilters );
    EXPECT_EQ( 5, conv->dim.filterSize.width );
//    EXPECT_EQ( "RELU", conv->activationFunction->getDefineName() );
